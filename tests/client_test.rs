...
fn create_server(addr: &str) -> Arc<Server> {
    Arc::new(Server::new(addr).expect("Failed to start server"))
}
...
fn test_client_echo_message() {
    // Set up the server in a separate thread
    let server = create_server("localhost:8081");
    let handle = setup_server_thread(server.clone());
...
fn test_multiple_echo_messages() {
    // Set up the server in a separate thread
    let server = create_server("localhost:8082");
    let handle = setup_server_thread(server.clone());
...
fn test_multiple_clients() {
    // Set up the server in a separate thread
    let server = create_server("localhost:8083");
    let handle = setup_server_thread(server.clone());
...
fn test_client_add_request() {
    // Set up the server in a separate thread
    let server = create_server("localhost:8084");
    let handle = setup_server_thread(server.clone());
